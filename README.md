# Hey, I'm Walter

**Product Engineer** — I bridge product thinking and technical execution.

I don't just write specs and hand them off. I build the thing, ship it, and iterate on it. My background in product management means I think about *why* before *how*, but I'm most at home when I'm deep in the code making it happen.

---

### What I'm building with

I work across the stack, with a focus on AI-native development:

- **AI & LLMs** — Building with the Claude ecosystem: API integrations, Claude Code, MCP servers, prompt engineering, and agentic workflows
- **Web** — TypeScript, React, Next.js, MDX
- **Backend** — Go (learning), Python, Node.js

### What I'm up to

- Studying for the **Claude Certified Architect** program
- Building AI-powered tools and workflows with Claude Code + MCP
- Shipping product features end-to-end in TypeScript

---

### Tech & Tools

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat-square&logo=go&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Anthropic](https://img.shields.io/badge/Anthropic-191919?style=flat-square&logo=anthropic&logoColor=white)
![Claude](https://img.shields.io/badge/Claude_Code-D97757?style=flat-square&logo=anthropic&logoColor=white)
![MDX](https://img.shields.io/badge/MDX-1B1F24?style=flat-square&logo=mdx&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
